...
class Shape() :
    def __init__(self) :
        ...
        t = [ [ 0, 1, 0 ],
              [ 1, 1, 1 ] ]
        shapes = [ square, horizontal_line, veritcal_line, left_l, right_l, left_s, right_s, t ]
...
win.listen()
win.onkeypress(lambda: shape.move_left(grid), 'a')
win.onkeypress(lambda: shape.move_right(grid), 'd')
...
while True :
    ...
    if shape.y == 23 - shape.height + 1 :
        shape = Shape()
        check_grid(grid)
    # check for the collision with the next row
